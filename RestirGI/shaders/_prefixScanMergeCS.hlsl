...
{
    const uint2 internal_sum = inout_buf.Load2(frame_constants.size.UINT_SIZE * (idx + segment * SEGMENT_SIZE));
    const uint prev_segment_sum = segment == 0 ? 0 : segment_sum_buf.Load(frame_constants.size.UINT_SIZE * (segment - 1));
...
{
    inout_buf.Store2(frame_constants.size.UINT_SIZE * (idx + segment * SEGMENT_SIZE), val);
}
