...
#include "image.hlsl"
#include "soft_color_clamp.hlsl"

Texture2D<float4> shadow_mask_tex : register(t0);
Texture2D<uint> bitpacked_shadow_mask_tex : register(t1);
Texture2D<float4> reprojection_tex : register(t2);

Texture2D<float4> prev_moments_tex : register(t3);
Texture2D<float4> prev_accum_tex : register(t4);

RWTexture2D<float2> temporal_output_tex : register(u0);
RWTexture2D<uint> meta_output_tex : register(u1);

RWTexture2D<float4> output_moments_tex : register(u2);
...
#include "ffx_denoiser_shadows_tileclassification.hlsl"

[numthreads(8, 8, 1)]
void CS(uint2 px : SV_DispatchThreadID, uint group_index: SV_GroupIndex, uint2 gid: SV_GroupID) {
