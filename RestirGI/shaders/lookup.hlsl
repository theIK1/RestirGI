...
struct IrcacheLookupMaybeAllocate
{
    ...
    Vertex proposal;
    bool just_allocated;
    static IrcacheLookupMaybeAllocate CreateNew(float3 testColor)
    {
        IrcacheLookupMaybeAllocate res;
        res.lookup.count = 0;
        res.lookup.entry_idx[0] = 0;
        res.lookup.weight[0] = 0;
        res.proposal.position = testColor;
        res.proposal.normal = float3(0, 0, 0);
        res.just_allocated = false;
        return res;
...
struct IrcacheLookupParams
{
    ...
    IrcacheLookupParams with_query_rank(uint v)
    {
        ...
        res.normal_ws = normal_ws;
        res.query_rank = v;
        res.stochastic_interpolation = stochastic_interpolation;
...
IrcacheLookupMaybeAllocate IrcacheLookupParams::lookup_maybe_allocate(inout uint rng)
{
    ...
    if (!IRCACHE_FREEZE)
    {
        ...
        //如果entry_flags为00或者01，那么just_allocated为false
        ...
        if (!skip_allocation)
        {
            //entry_flags为未分配(00)或者是分配状态(10)进入
