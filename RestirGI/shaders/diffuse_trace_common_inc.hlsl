...
TraceResult newTraceResult()
{
    ...
    traceresult.out_value = float3(0, 0, 0);
    traceresult.hit_normal_ws = float3(0, 0, 0);
    traceresult.hit_t = 0;
    traceresult.pdf = 0;
    traceresult.is_hit = false;
    return traceresult;
...
TraceResult do_the_thing(uint2 px, float3 normal_ws, inout uint rng, RayDesc outgoing_ray) {
    ...
    if (primary_hit.is_hit) {
        ...
        {
            if ((frame_constants.size.pad0 & (1 << 0)) == (1 << 0))
            {
