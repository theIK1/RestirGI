#ifndef LAYERED_BRDF_HLSL
#define LAYERED_BRDF_HLSL
...
void apply_metalness_to_brdfs(inout SpecularBrdf specular_brdf, inout DiffuseBrdf diffuse_brdf, float metalness) {
    ...
    specular_brdf.albedo = lerp(specular_brdf.albedo, albedo, metalness); //金属度越大，高光越大
    diffuse_brdf.albedo = max(0.0, 1.0 - metalness) * albedo; //金属度越大，漫反射项越小

    const float3 albedo_boost = metalness_albedo_boost(metalness, albedo); //如果金属度不是0或1，会由于漫反射项损失能量，需要缩放一下
...
struct LayeredBrdf {
    ...
    static LayeredBrdf from_gbuffer_ndotv(
        ...
        SpecularBrdf specular_brdf;
        specular_brdf.albedo = 0.04; //初始高光
    ...
    float3 evaluate(float3 wo, float3 wi) {
        ...
        return ( 
            ...
            diff.value * spec.transmission_fraction //漫反射项 * (1-F)  ，F为菲涅尔相
        );
    }
