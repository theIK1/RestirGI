...
Texture2D<float4> half_view_normal_tex : register(t2);
Texture2D<float> half_depth_tex : register(t3);
Texture2D<float4> ssao_tex : register(t4);
Texture2D<float4> candidate_radiance_tex : register(t5);
Texture2D<float4> candidate_hit_tex : register(t6);
Texture2D<uint4> temporal_reservoir_packed_tex : register(t7);

Texture2D<float4> radiance_tex : register(t8);
Texture2D<uint2> reservoir_input_tex : register(t9);
Texture2D<float3> bounced_radiance_input_tex : register(t10);

RWTexture2D<float4> irradiance_output_tex : register(u0);
...
void CS(uint2 px : SV_DispatchThreadID) {
    ...
    for (uint sample_i = 0; sample_i < (RTDGI_RESTIR_USE_RESOLVE_SPATIAL_FILTER ? 4 : 1); ++sample_i) {
...

        {
            ...
            const float3 sample_offset = hit_ws - view_ray_context.ray_hit_ws();//´òßã£¡
            const float sample_dist = length(sample_offset);
            const float3 sample_dir = sample_offset / sample_dist;
