...
#include "working_color_space.hlsl"
...
Texture2D<float4> history_tex : register(t1);
Texture2D<float4> reprojection_tex : register(t2);

Texture2D<float2> variance_history_tex : register(t3);
Texture2D<float2> rt_history_invalidity_tex : register(t4);

RWTexture2D<float4> output_tex : register(u0);

RWTexture2D<float4> history_output_tex : register(u1);
RWTexture2D<float2> variance_history_output_tex : register(u2);
...
{
    if (indirectCB.enableTemporal == 0)
    {
