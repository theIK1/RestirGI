...
Texture2D<float3> half_view_normal_tex : register(t0);
Texture2D<float> depth_tex : register(t1);
Texture2D<float4> candidate_radiance_tex : register(t2);
Texture2D<float3> candidate_normal_tex : register(t3);
Texture2D<float4> candidate_hit_tex : register(t4);
Texture2D<float4> reprojection_tex : register(t5);

Texture2D<float4> radiance_history_tex : register(t6);
Texture2D<float3> ray_orig_history_tex : register(t7);
Texture2D<float4> ray_history_tex : register(t8);
Texture2D<uint2> reservoir_history_tex : register(t9);
Texture2D<float4> hit_normal_history_tex : register(t10);
Texture2D<float4> candidate_history_tex : register(t11);
Texture2D<float2> rt_invalidity_tex : register(t12);

RWTexture2D<uint4> temporal_reservoir_packed_tex : register(u0);
RWTexture2D<uint4> RWTest : register(u1);

RWTexture2D<float4> radiance_out_tex : register(u2);
RWTexture2D<float3> ray_orig_output_tex : register(u3);
RWTexture2D<float4> ray_output_tex : register(u4);
RWTexture2D<float4> hit_normal_output_tex : register(u5);
RWTexture2D<uint2> reservoir_out_tex : register(u6);
RWTexture2D<float4> candidate_out_tex : register(u7);
...
void CS(uint2 px : SV_DispatchThreadID) {
    ...
    const uint reservoir_payload = px.x | (px.y << 16); //record pixel position
    ...
    if (is_rtdgi_tracing_frame()) {
        ...
        //mix now(95%) and history(5%) hit length
    ...
    if (use_resampling) {
        for (
            uint sample_i = 0;
            sample_i < MAX_RESOLVE_SAMPLE_COUNT //5
            ...
            //permuted_reproj_px = px;

             int2 rpx = permuted_reproj_px + rpx_offset; //const
        ...
        // Increases noise, but prevents leaking in areas of geometric complexity
            ...
            //hit point to current ray origin(offset)
            ...
            
            {
                ...
                //          (history hit point - history ray origin)    (history hit point - current ray origin(offset))
