...
Texture2D<float4> half_view_normal_tex : register(t0);
Texture2D<float> half_depth_tex : register(t1);
Texture2D<float> depth_tex : register(t2);
Texture2D<float> half_ssao_tex : register(t3);
Texture2D<uint4> temporal_reservoir_packed_tex : register(t4);
Texture2D<float3> reprojected_gi_tex : register(t5);

Texture2D<uint2> reservoir_input_tex : register(t6);
Texture2D<float3> bounced_radiance_input_tex : register(t7);

RWTexture2D<uint2> reservoir_output_tex : register(u0);
RWTexture2D<float3> bounced_radiance_output_tex : register(u1);

ConstantBuffer<RestirSpatialCB> restirSpatialCB : register(b1);
...
void CS(uint2 px : SV_DispatchThreadID) {
    ...
    const uint seed = frame_constants.frame_index + restirSpatialCB.spatial_reuse_pass_idx * 123;
    uint rng = hash3(uint3(px, seed));

    const float2 uv = get_uv(hi_px, restirSpatialCB.gbuffer_tex_size);
    ...
    const float MAX_INPUT_M_IN_PASS = restirSpatialCB.spatial_reuse_pass_idx == 0 ? MAX_INPUT_M_IN_PASS0 : MAX_INPUT_M_IN_PASS1;
    ...
    float max_kernel_radius =
        restirSpatialCB.spatial_reuse_pass_idx == 0
    ...
    if (restirSpatialCB.spatial_reuse_pass_idx >= 2)
    {
        max_kernel_radius = 8;
    }

    const float2 dist_to_edge_xy = min(float2(px), restirSpatialCB.output_tex_size.xy - px);
    ...
    uint sample_count = DIFFUSE_GI_USE_RESTIR
        ? (restirSpatialCB.spatial_reuse_pass_idx == 0 ? SAMPLE_COUNT_PASS0 : SAMPLE_COUNT_PASS1)
        : 1;
    ...
    const uint2 ang_offset_seed = restirSpatialCB.spatial_reuse_pass_idx == 0
            ? (px >> 3)
            : (px >> 2);
    #else
    ...
    float ang_offset = uint_to_u01_float(hash3(
        uint3(ang_offset_seed, frame_constants.frame_index * 2 + restirSpatialCB.spatial_reuse_pass_idx)
    )) * M_PI * 2;
    ...
    for (uint sample_i = 0; sample_i < sample_count; ++sample_i) {
        ...
        const float2 rpx_uv = get_uv(
            ...
            restirSpatialCB.gbuffer_tex_size);
        ...
        if (RTDGI_RESTIR_SPATIAL_USE_RAYMARCH && restirSpatialCB.perform_occlusion_raymarch )
        {
            ...
            OcclusionScreenRayMarch raymarch = OcclusionScreenRayMarch::create(
                ...
                raymarch_end_ws,
                restirSpatialCB.gbuffer_tex_size.xy
            )
            .with_max_sample_count(6)
            .with_halfres_depth(restirSpatialCB.output_tex_size.xy, half_depth_tex);
        ...
        if ((frame_constants.size.pad0 & (1 << 3)) != (1 << 3))
        {
            jacobian = 1;
        ...
        if (restirSpatialCB.occlusion_raymarch_importance_only)
        {
