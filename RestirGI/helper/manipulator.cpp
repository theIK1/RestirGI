...
void Manipulator::sunMove(int x, int y)
{
    float dx = float(x - m_mouse[0]) / float(m_width);
    float dy = float(y - m_mouse[1]) / float(m_height);

    m_sun += glm::vec3(-dy * 10, 0, dx * 10);
...
void Manipulator::translate(bool* m_keys, float m_cameraStepMove)
{
    // auto forward = glm::normalize(m_int - m_pos);
    auto forward = glm::normalize(m_int - m_pos);
    forward.x *= m_cameraStepMove;
    forward.y *= m_cameraStepMove;
    forward.z *= m_cameraStepMove;

    auto strafe = glm::normalize(glm::cross(m_up, forward));
    strafe.x *= m_cameraStepMove;
    strafe.y *= m_cameraStepMove;
    strafe.z *= m_cameraStepMove;
    if (m_keys['W'])
    {
        m_pos += forward;
        m_int += forward;
